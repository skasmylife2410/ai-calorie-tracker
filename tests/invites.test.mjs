...
let DB;
const reset = () => { DB = { snapcal_users: [{ username: "aelson" }, { username: "baby" }, { username: "thayra23" }], snapcal_invites: [] }; };
...
test("an invite link lets exactly one person in, once", async () => {
  reset();
  let res = mockRes(); await invites(as("aelson", { op: "create" }), res);
...
test("only the owner can create or cancel links", async () => {
  reset();
  let res = mockRes(); await invites(as("baby", { op: "create" }), res);
  ...
  res = mockRes(); await invites(anon({ op: "create" }), res);
...
test("five open links fill the five free places; a sixth can't be made", async () => {
  reset();
  for (let i = 0; i < 5; i++) {
    const r = mockRes(); await invites(as("aelson", { op: "create" }), r);
    assert.equal(r.body.ok, true, `link ${i + 1}`);
  }
  const sixth = mockRes(); await invites(as("aelson", { op: "create" }), sixth);
  assert.equal(sixth.body.errorType, "full");

  // cancelling one frees a place again
  const code = DB.snapcal_invites[0].code;
  await invites(as("aelson", { op: "revoke", code }), mockRes());
  const again = mockRes(); await invites(as("aelson", { op: "create" }), again);
